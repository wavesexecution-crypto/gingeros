...
export default async function Followups() {
  ...
  const overdue = await db.prepare(`${sel} AND NULLIF(f.due_date, '')::date < CURRENT_DATE ORDER BY f.due_date`).all() as unknown as Fup[];
  const today = await db.prepare(`${sel} AND NULLIF(f.due_date, '')::date = CURRENT_DATE ORDER BY f.id`).all() as unknown as Fup[];
  const upcoming = await db.prepare(`${sel} AND NULLIF(f.due_date, '')::date > CURRENT_DATE ORDER BY f.due_date LIMIT 100`).all() as unknown as Fup[];
