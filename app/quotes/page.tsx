...
export default async function Quotes({ searchParams }: { searchParams: Promise<Record<string, string>> }) {
  ...
  const db = getDb();
  const rows = (status
    ? await db.prepare("SELECT q.*, co.name AS cname FROM quotes q JOIN companies co ON co.id=q.company_id WHERE q.status=? ORDER BY q.id DESC").all(status)
    : await db.prepare("SELECT q.*, co.name AS cname FROM quotes q JOIN companies co ON co.id=q.company_id ORDER BY q.id DESC").all()) as Record<string, unknown>[];
  return (
